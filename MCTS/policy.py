...
def _convolve_board_available_3(board_array):
    convolved_1 = sig.convolve2d(board_array[:, :, 0], __available_kernel_3, 'same')
    convolved_2 = sig.convolve2d(board_array[:, :, 1], __available_kernel_3, 'same')
    return np.array(convolved_1 + convolved_2 > 0, dtype=np.int).reshape((15 * 15,))


def _convolve_board_available_2(board_array):
    convolved_1 = sig.convolve2d(board_array[:, :, 0], __available_kernel_2, 'same')
    convolved_2 = sig.convolve2d(board_array[:, :, 1], __available_kernel_2, 'same')
    return np.array(convolved_1 + convolved_2 > 0, dtype=np.int).reshape((15 * 15,))


def _convolve_board_available_1(board_array):
    convolved_1 = sig.convolve2d(board_array[:, :, 0], __available_kernel_1, 'same')
    convolved_2 = sig.convolve2d(board_array[:, :, 1], __available_kernel_1, 'same')
    return np.array(convolved_1 + convolved_2 > 0, dtype=np.int).reshape((15 * 15,))


def expand_pocliy_random(board: Board):
    board_array = board.get_board_array()
    conv_available = _convolve_board_available_2(board_array)
...
def rollout_policy_random(board: Board):
    board_array = board.get_board_array()
    conv_available = _convolve_board_available_1(board_array)
...
def expand_policy_network(board: Board) -> List[Tuple[int, float]]:
    ...
    conv_available = _convolve_board_available_3(board_array)
    return_list = []
